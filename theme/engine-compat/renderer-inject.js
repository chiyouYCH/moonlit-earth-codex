...
((cssText, artDataUrl, themeConfig) => {
  ...
  const ancestorNodes = (node, maxDepth = 14) => {
    const nodes = [];
    for (let current = node; current && nodes.length < maxDepth;
      current = current.parentElement) nodes.push(current);
    return nodes;
  };
  const largestAncestor = (node, predicate) => ancestorNodes(node)
    .filter((candidate) => {
      try { return predicate(candidate, candidate.getBoundingClientRect()); } catch { return false; }
    })
    .sort((left, right) => {
  ...
  const resolvedSummaryPanelNodes = () => {
    const nodes = new Set();
    for (const anchor of genericNodes('[data-slot^="thread-summary-panel-"]')) {
      const owner = largestAncestor(anchor, (candidate, rect) =>
        candidate.classList?.contains("bg-surface-elevated-secondary") &&
        rect.width >= 220 && rect.height >= 120 && rect.width < innerWidth * .6);
      if (owner) nodes.add(owner);
    }
    return [...nodes];
  };
  const resolvedTerminalPanelNodes = () => genericNodes('[id^="terminal-panel-"]');
  const resolvedBottomPanelNodes = (terminalNodes) => {
    const nodes = new Set();
    for (const terminal of terminalNodes) {
      const owner = ancestorNodes(terminal).find((candidate) =>
        candidate.classList?.contains("z-30") &&
        candidate.classList?.contains("w-full") &&
        candidate.classList?.contains("shrink-0"));
      if (owner) nodes.add(owner);
    }
    return [...nodes];
  };
  const resolvedSidePanelNodes = () => {
    // Codex 26.831 renamed the width controls to Enter/Exit full screen.
    // Use the semantic pane owner, independent of locale, controls, or tabs.
    const nodes = new Set(genericNodes('[data-app-shell-focus-area="right-panel"]'));
    // Keep older shells working without broadening the rule to other asides.
    for (const node of genericNodes('aside:not(.app-shell-left-panel):not([data-app-shell-focus-area])')) {
      if (node.querySelector?.(
        'button[aria-label="展开面板"], button[aria-label="折叠面板"], ' +
        'button[aria-label*="expand panel" i], button[aria-label*="collapse panel" i]',
      )) nodes.add(node);
    }
    return [...nodes];
  };
  const resolvedSettingsNodes = () => {
    const activeItem = selectorNodes("settings-panel")[0] ??
      genericNodes('[data-settings-panel-slug]')[0];
    if (!activeItem) return { sidebar: [], content: [] };
    const sidebar = activeItem.closest?.('.app-shell-left-panel');
    const viewport = sidebar?.closest?.('.zoom-adjusted-viewport') ??
      activeItem.closest?.('.zoom-adjusted-viewport');
    const content = viewport ? [...viewport.children].find((node) =>
      node !== sidebar && !node.contains?.(activeItem) &&
      node.getBoundingClientRect?.().width >= 320) : null;
    return {
  ...
  const refreshParts = () => {
    ...
    const terminalNodes = resolvedTerminalPanelNodes();
    const settingsNodes = resolvedSettingsNodes();
    addPart(desired, "summary-panel", resolvedSummaryPanelNodes());
    addPart(desired, "bottom-panel", resolvedBottomPanelNodes(terminalNodes));
    addPart(desired, "terminal-panel", terminalNodes);
    addPart(desired, "side-panel", resolvedSidePanelNodes());
    addPart(desired, "settings-sidebar", settingsNodes.sidebar);
    addPart(desired, "settings-content", settingsNodes.content);
